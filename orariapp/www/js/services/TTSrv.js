angular.module('viaggia.services.timetable', [])
	...
	.factory('ttService', function ($http, $rootScope, $q, $filter, Config, DataManager, GeoLocate) {
		...
		var dataFromHash = function (agency, route, date, deferred, readRoutes) {
			...
			DataManager.doQuery("SELECT * FROM route WHERE agencyID = '" + agency + "' AND linehash = '" + hash + "'", []).then(function (data) {
		...
		var getNextTrips = function (agencyId, stopId, numberOfResults) {
			...
			$http.get(Config.getServerURL() + '/getlimitedtimetable/' + agencyId + '/' + stopId + '/' + numberOfResults, Config.getHTTPConfig()).success(function (data) {
			...
			getTT: function (agency, route, date) {
				...
				if (ionic.Platform.isWebView()) {
					...
					if (calendarCache[agency][route] == null) {
						DataManager.doQuery("SELECT calendar FROM calendar WHERE agencyID = '" + agency + "' AND route = '" + route + "'", []).then(function (data) {
