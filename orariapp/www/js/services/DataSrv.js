angular.module('viaggia.services.data', []).factory('DataManager', function ($http, $q, $cordovaSQLite, $cordovaFile, Config) {
	...
	var process = function (url) {
		...
		JSZipUtils.getBinaryContent(url, function (err, data) {
			...
			Object.keys(jszipobj.files).forEach(function (key) {
				$cordovaFile.createFile(getDBPath(), getDBFileShortName(), true).then(function (success) {
					var f = jszipobj.file(key);
					$cordovaFile.writeFile(getDBPath(), getDBFileShortName(), jszipobj.file(key).asArrayBuffer(), true).then(function (success) {
	...
	var synchDB = function () {
		...
		getLocalVersion().then(function (localversion) {
			$http.get(Config.getServerURL() + '/versions').success(function (remoteversion) {
	...
	var syncStopsForVersions = function (remoteversion) {
		...
		agencies.forEach(function (a) {
			...
			if (!localStops || localVersion < remoteVersion) {
				$http.get(Config.getServerURL() + '/geostops/' + a + '?lat=' + Config.getMapPosition().lat + '&lng=' + Config.getMapPosition().long + '&radius=5').success(function (stops) {
